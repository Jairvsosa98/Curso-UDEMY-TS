        ...
        activities.map((activity) => (
          <div key={activity.id} className="px-5 py-10 bg-white mt-5 flex justify-between shadow">
            <div className="space-y-2 relative">
              <p className={`absolute -top-8 -left-8 px-10 py-2 text-white uppercase font-bold ${activity.category === 1 ? 'bg-lime-600' : 'bg-orange-600'}`}>
                {categoryName(+activity.category)}
              </p>
              <p className="text2xl font-bol pt-5">{activity.name}</p>
              <p className="font-black text-4xl text-lime-600">{activity.calories} {' '}
                <span>Calorias</span>
              </p>
            </div>
