...
export default function PatientForm() {
    ...
    const { register, handleSubmit } = useForm<DraftPatient>({
        mode: 'onSubmit',
        ...
        Object.entries(formErrors).forEach(([field, err]) => {
            ...
            if (err.types) {
                Object.values(err.types).forEach((msg) =>
                    toast.warning(String(msg), { position: 'bottom-right', toastId: `${field}-${msg}` })
                )
            } else if (err.message) {
                toast.warning(String(err.message), { position: 'bottom-right', toastId: field })
            }
        })
    }
